...
if kubectl rollout status deployment/product-deployment -n prod | grep -q "successfully rolled out"; then
  echo "Success to create new kubernetes deployment resources"

  kubectl annotate deployment/product-deployment kubernetes.io/change-cause="Change image to ${IMAGE_TAG}"
  ...
  kubectl rollout undo deployment/product-deployment -n prod
  if kubectl rollout status deployment/product-deployment -n prod | grep -q "successfully rolled out"; then
    echo "Rollback was successful"

    kubectl annotate deployment/product-deployment kubernetes.io/change-cause="Rollback to previous image"
  else
